const RegisterEventService = {
  ...
  init: function() { // 익명함수의 this는 해당 메소드를 소유한 객체에 바인딩.
  ...
  addCategorySelectEvent: function() {
    this.getCategorySelectObj().onchange = () => { // onchange select 태그 변경 시
  ...
  addNameInputEvent: function() {
    this.getNameInputObj().onkeyup = () => { // onkeyup 키보드 키 눌렀다 땠을 때
