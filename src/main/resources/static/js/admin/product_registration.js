class ProductMst { // js는 자기자신을 가르키는 this를 써야함
  #category; //#은 private
  #name;
  #price;
  #simpleInfo;
...
class RegisterEventService {
  ...
  addCategorySelectEvent() {
    this.#categorySelectObj.onchange = () => { // onchange select 태그 변경 시
  ...
  addNameInputEvent() {
    this.#nameInputObj.onkeyup = () => { // onkeyup 키보드 키 눌렀다 땠을 때
...
class RegisterService { // 싱글톤 패턴 : 메모리 영역에서 한번의 new로 인스턴스를 사용하기 때문에 메모리 방지 가능.
